...
class Splitter(object):
    def __init__(self, splits, random_state_seed):
        ...
        indices = OrderedDict(unlabel=10)
        indices.update(splits)
        self.splits = indices
...
class ClassSplitter_(Splitter):
    def __init__(self, shuffle=True, num_samples_per_class=None,
                 ...
                 num_support_per_class=None, num_query_per_class=None, ratio = 1.0, num_distractor_classes = 0, 
                 random_state_seed=0):
        ...
        self.num_distractor_classes = num_distractor_classes
    ...
    def get_indices_concattask(self, task):
        ...
        for dataset in task.datasets:
            ...
            if class_ind < len(task.datasets) - self.num_distractor_classes:
                if self.shuffle:
                    seed = (hash(task) + self.random_state_seed) % (2 ** 32)
                    num_label = int(num_samples*self.ratio)
                    num_unlabel = num_samples - num_label
                    label_indices = np.random.permutation(np.arange(1, num_label))
                    unlabel_indices = np.random.permutation(np.arange(num_label, num_samples))

                else:
                    num_label = int(num_samples*self.ratio)
                    num_unlabel = num_samples - num_label
                    label_indices = np.arange(1, num_label)
                    unlabel_indices = np.arange(num_label, num_samples)


                unlabel_split = self.splits['unlabel']
                ptr = 0
                for split, num_split in self.splits.items():
                    if split != 'unlabel':
                        split_indices = label_indices[ptr:ptr + num_split]
                        if self.shuffle:
                            self.np_random.shuffle(split_indices)
                        indices[split].extend(split_indices + cum_size)
                        ptr += num_split
                   
                unlabel_ids = unlabel_indices[0:unlabel_split]
                indices['unlabel'].extend(unlabel_ids + cum_size)   
                ...
                distractor_split = self.distractor
                distractor_ids = unlabel_indices[0:distractor_split]
                indices['unlabel'].extend(distractor_ids + cum_size)
